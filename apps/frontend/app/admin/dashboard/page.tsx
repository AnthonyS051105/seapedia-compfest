  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {!stats || !statCards ? (
        ...
        <Reveal stagger staggerGap={0.06} className="grid grid-cols-2 gap-4 lg:grid-cols-5">
          {statCards.map(({ label, value, icon: Icon, danger }, index) => {
            const cardContent = (
              <div
                className={cn(
            ...
            return (
              <RevealItem key={label}>
                {index < 2 ? <TiltCard maxTilt={6}>{cardContent}</TiltCard> : cardContent}
              </RevealItem>
            )
          })}
        </Reveal>
      ...
      {stats && (
        <Reveal>
          <div className="rounded-xl border border-zinc-200 bg-white p-5 dark:border-zinc-800 dark:bg-zinc-900">
            ...
            <Reveal stagger staggerGap={0.04} className="grid grid-cols-2 gap-3 sm:grid-cols-5">
              {Object.entries(stats.orders.by_status).map(([status, count]) => (
                <RevealItem key={status}>
