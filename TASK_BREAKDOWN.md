...
- [ ] `app/seller/dashboard/page.tsx` — stats + recent orders summary
- [ ] `app/seller/store/page.tsx` — store profile form (create/edit)
  - Check if store exists on mount
  - If no store → show create form
  - If store exists → show edit form

- [ ] `app/seller/products/page.tsx` — product list with CRUD
- [ ] `app/seller/products/new/page.tsx` — create product form
...
- [ ] `app/seller/orders/page.tsx` — seller incoming orders
...
- [ ] Process order button in seller order detail/list
- [ ] Confirmation modal before processing
- [ ] Status badge updates after processing
...
- [ ] Vertical stepper with timestamps
- [ ] Different icons per status
- [ ] Completed/current/pending visual states
- [ ] Used on buyer order detail + seller order detail
